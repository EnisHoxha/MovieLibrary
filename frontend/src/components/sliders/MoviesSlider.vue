...
const getMovies = async () => {
  await axios
    .get(`${host}/api/movies/typeMovie`, { withCredentials: true })
    .then((res) => {
...
<template>
  <section class="md:p-12 p-6 ">
    <h1 class="md:text-4xl font-medium dark:text-white border-l-4 border-yellow-500 my-2 pl-2">Watch Movies</h1>
      ...
      <swiper-slide v-for="movie in movies.movie" :key="movie.id">
        <div>
          <router-link :to="`/movies/${movie._id}`">
            <img :src="'http://localhost:5002/static/'+ movie.poster_img" class="h-52 w-full object-fill rounded-sm hover:opacity-75   tansition easy-in-out duration-150" />
          </router-link>
